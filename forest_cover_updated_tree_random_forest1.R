...
library(caret)
library(ggplot2)
my.path <- 'C:\\Users\\Abhinav\\Desktop\\Fiverr\\Ali\\Advanced_Data_Analytics\\Project1\\Forest_Cover_Package\\';
...
forest.cover$Cover_Name = as.factor(forest.cover$Cover_Name)
forest.cover$Wilderness_Area = as.factor(forest.cover$Wilderness_Area)

plot_missing(forest.cover)
#Here we see that there are no missing values in our dataset 

##############################Spliting the data in train, Validate and test data set 
train = forest.cover[forest.cover$train==1,]
test = forest.cover[forest.cover$test==1,]
...
library(rpart)
library(rpart.plot)
model.tr = rpart(Cover_Name ~ Elevation+Aspect+Slope+HDist_Hydrology+VDist_Hydrology+HDist_Roadways+Hillshade_9am+Hillshade_Noon
                 +Hillshade_3pm+HDist_Fire_Points+Wilderness_Area+soil_family+soil_rock_type,
                 data = train)
rpart.plot(model.tr)
pred.tr = predict(model.tr,train,type = "class")
pred.tr_test = predict(model.tr,test,type = "class")
confusionMatrix(pred.tr,train$Cover_Name)
## We obtain an accuracy of 67.69%% on train data 
confusionMatrix(pred.tr_test,test$Cover_Name)
##We obtain an accuracy of 67.47%% on test data
...
set.seed(3000)
model.rf = randomForest(Cover_Name ~ Elevation+Aspect+Slope+HDist_Hydrology+VDist_Hydrology+HDist_Roadways+Hillshade_9am+Hillshade_Noon
                        +Hillshade_3pm+HDist_Fire_Points+Wilderness_Area+soil_family+soil_rock_type,
                        data = train)
model.rf
varImp(model.rf)
varImpPlot(model.rf,type=2)
pred.rf = predict(model.rf,train,type = "class")
pred.rf_test = predict(model.rf,test,type = "class")
confusionMatrix(pred.rf,train$Cover_Name)
## We obtain an accuracy of 100% on train data as this the data on which we trained our model
confusionMatrix(pred.rf_test,test$Cover_Name)
##We obtain an accuracy of 94.92% on test data
